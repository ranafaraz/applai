        ...
        <form method="POST" action="{{ route('email-templates.update', $emailTemplate) }}" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $emailTemplate->name) }}" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                    ...
                    <select name="type" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        @foreach(['initial_outreach'=>'Initial Outreach','follow_up'=>'Follow-up','thank_you'=>'Thank You','networking'=>'Networking','other'=>'Other'] as $val=>$label)
                            <option value="{{ $val }}" {{ old('type', $emailTemplate->type) === $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div>
                ...
                <input type="text" name="subject" value="{{ old('subject', $emailTemplate->subject) }}" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Body <span class="text-red-500">*</span></label>
                <textarea name="body" rows="15" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('body', $emailTemplate->body) }}</textarea>
            </div>
            <div class="flex gap-3 pt-2">
                ...
                <form method="POST" action="{{ route('email-templates.destroy', $emailTemplate) }}" class="ml-auto" onsubmit="return confirm('Delete this template?')">
                    @csrf @method('DELETE')
