@extends('layouts.app')
@section('title', 'Contact Imports')
@section('page-title', 'Contact Imports')
@section('content')
<div>
    <div class="flex items-center justify-between mb-5">
        <p class="text-sm text-slate-500">Import contacts from CSV files</p>
        <a href="{{ route('imports.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2 rounded-lg flex items-center gap-2">
    ...
    <div class="bg-white border border-slate-200 rounded-xl overflow-hidden">
        @if($imports->isEmpty())
            <div class="text-center py-16">
                ...
                <a href="{{ route('imports.create') }}" class="mt-2 inline-block text-sm text-indigo-600 hover:underline">Upload your first CSV</a>
            </div>
        @else
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @php $statusColors = ['pending'=>'yellow','processing'=>'blue','completed'=>'green','failed'=>'red']; $sc = $statusColors[$import->status] ?? 'gray'; @endphp
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('imports.show', $import) }}" class="text-xs text-indigo-600 hover:underline px-2 py-1 rounded hover:bg-slate-100">Details</a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
